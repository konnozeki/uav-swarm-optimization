...
def test_reconfiguration_statistics_compare_all_baselines(tmp_path: Path):
    ...
    blocked = pd.read_csv(tmp_path / "statistics_blocked_by_problem.csv")
    assert set(blocked["baseline"]) == {"baseline_a", "baseline_b"}
    assert set(blocked["n_problems"]) == {1}
    assert set(blocked["n_seed_pairs"]) == {4}
